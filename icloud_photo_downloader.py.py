...
class iCloudPhotoDownloader:
    ...
    def download_file(self, photo, folder_path, file_name):
        """
        Downloads a single file and saves it to the specified folder.
        """
        try:
    ...
    def test_mode(self):
        """
        Test mode: Downloads the first item in the library into a 'Test' folder.
        """
        ...
        try:
            ...
            if api.requires_2fa:
                code = messagebox.askstring("Two-Factor Authentication", "Enter the 2FA code sent to your devices:")
    ...
    def start_download(self):
        """
        Full download: Downloads all items into folders organized by month and year.
        """
        apple_id = self.apple_id.get()
        password = self.password.get()
        download_dir = self.download_dir.get()
